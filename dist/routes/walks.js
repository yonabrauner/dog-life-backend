...
walksRouter.post("/", async (req, res) => {
    const { date, duration, walkerName, notes, activities } = req.body;
    try {
        console.log("post request recieved from: ", walkerName, " at: ", date);
        const walk = await createWalk({ date, duration, walkerName, notes, activities });
        res.json(walk);
    }
    catch (err) {
        console.error(err);

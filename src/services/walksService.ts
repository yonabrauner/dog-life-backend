...
export async function getDogsForWalk(walkId: number) {
...
export async function getLastWalkWithActivity(dogName: string, activity: 'poop' | 'pee'){
...
export async function createWalk({date, duration, walkerName, notes, dogActivities // Array of { dogName, pee, poop }
  ...
  walkerName: string,
  notes?: string,
  dogActivities: { dogName: string; pee: boolean; poop: boolean }[]
}) {
  const finalDogActivities = await Promise.all(
    dogActivities.map(async (activity) => {
